...
void firFloat(double * coeffs, double * input, double * output,
  ...
  q.submit([&](auto &h){
    // auto out = stream(16384, 768, h);
    h.parallel_for(nd_range<1>(N, P), [=](auto item){
      ...
      for (int k = 0; k < filterLength; k++) {
        if(i-k >= 0)
        {
          output[i] += coeffs[k] * input[i-k];
        }
        // out << "output[" << i << "] = " << coeffs[k] << "["<< k << "]" << " x " << input[i+k] << "["<< i+k << "]" << "\n";
      }


      //# query sub_group and print sub_group info once per sub_group
      // if (wg.get_local_id()[0] == 0) {
      //   // out << "work_group id: " << wg.get_group_id()[0] << " of "
      //   //     << wg.get_group_range()[0] << ", size=" << wg.get_local_range()[0]
      //   //     << "\n";
